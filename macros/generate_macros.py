...
with open(hfile, "w") as h:
	...
	for section in config.sections():
		...
		for bits, name in config.items(section):
			...
			h.write(template_c.format(command_format.format(section, name), "\n".join(commands)))

	# interrupts
	h.write("\n// interrupts\n\n")
	for name, bit_name in config.items("interrupts"):
		for enable_disable, bitvalue in (("enable", "1"), ("disable", "0")):
			h.write(template_c.format(command_format.format("int_"+name, enable_disable), bit_commands[bitvalue].format(bit_registers[bit_name], bit_name)))
